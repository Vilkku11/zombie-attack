...
func can_partially_merge_with(other_slot_data: SlotData) -> bool:
	return item_data == other_slot_data.item_data \
		and item_data.stackable \
		and not can_fully_merge_with(other_slot_data)

func partially_merge_with(other_slot_data: SlotData) -> void:
	
	other_slot_data.set_quantity(quantity + other_slot_data.quantity - MAX_STACK_SIZE)
